...
class Minify_YUICompressor {
    ...
    public static $jarFile =  "/../library/YUI/yuicompressor-2.4.2.jar";
     ...
     * @var string
     */
    public static $javaExecutable = '/usr/bin/java';
    ...
    {
        self::_prepare();
        if (! ($tmpFile = tempnam(getcwd() . self::$tempDir, 'yuic_'))) {
    ...
    private static function _getCmd($userOptions, $type, $tmpFile)
    {
        ...
        $cmd = self::$javaExecutable . ' -jar ' . escapeshellarg(getcwd() . self::$jarFile)
             . " --type {$type}"
    ...
    private static function _prepare()
    {
        ...
        if (! is_file(getcwd() . self::$jarFile) 
            || ! is_dir(getcwd() . self::$tempDir)
            || ! is_writable(getcwd() . self::$tempDir)
        ) {
